...
var Epsile = new (function () {
  ...
  function createConnection() {
    ...
    socket.on("conn", function () {
      // Connected
      chatMainDiv.innerHTML = "";
      logChat(0, "<br/>You are now chatting with a random stranger. Say hi!");
    ...
    socket.on("disconnect", function () {
      ...
      logChat(
        -1,
        "<input type=button value='Reconnect' onclick='Epsile.startChat();'>"
      );
    ...
    if (isBlurred && (type === 0 || type === 2)) {
      ...
      if (
        ...
        lastNotify = window.webkitNotifications.createNotification(
          "img/epsile_logo32.png",
          "Epsile" + (type === 0 ? " Message" : ""),
          who2 + message2
        );
        lastNotify.show();
        firstNotify = false;
  ...
  // send message button
  document.getElementById("sendmsg").addEventListener(
    "click",
    function () {
